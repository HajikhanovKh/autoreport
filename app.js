...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function loadSigners() {
        fetch(SIGNER_API_URL).then(r => r.json()).then(data => {
            if (data && data.length > 0) {
                ...
                if(iLeaderPerson) iLeaderPerson.value = s.leaderperson || ''; 
                if(iLeaderName) iLeaderName.value = s.leadername || '';
                if(iSecondPerson) iSecondPerson.value = s.secondperson || ''; 
                if(iPhone) iPhone.value = s.phone || '';
            }
        }).catch(err => console.error(err));
    }
    
    if (signerBtn && popupSigners) { signerBtn.addEventListener('click', e => { e.preventDefault(); popupSigners.style.display = 'flex'; if(signerStatusMsg) signerStatusMsg.style.display = 'none'; }); }
    ...
    if (saveSignersBtn) {
        saveSignersBtn.addEventListener('click', e => {
            ...
            fetch(url, { method: method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
                ...
                if(signerStatusMsg) { signerStatusMsg.innerText = "Uğurla yadda saxlanıldı! ✅"; signerStatusMsg.style.color = "#10b981"; signerStatusMsg.style.display = "block"; }
                if (!currentSignerId && data && data.id) currentSignerId = data.id;
                setTimeout(() => { popupSigners.style.display = 'none'; }, 1500);
            }).catch(err => {
                if(signerStatusMsg) { signerStatusMsg.innerText = "Xəta baş verdi!"; signerStatusMsg.style.color = "#ef4444"; signerStatusMsg.style.display = "block"; }
    ...
    function renderBildirisTable(highlightId = null) {
        ...
        if (highlightId) {
            ...
            if (targetIndex !== -1) currentBilPage = Math.floor(targetIndex / bilRowsPerPage) + 1;
        }
        ...
        pageData.forEach(b => {
            ...
            if (highlightId && b.id.toString() === highlightId.toString()) tr.classList.add('highlight-row');
    ...
    window.updateBildirisFromTable = function(id) {
        ...
        if(!newNomre) { alert("Bəyannamə/Bildiriş nömrəsini daxil edin!"); return; }
        ...
        }).then(() => { loadAllBildirisler(() => renderBildirisTable()); }).catch(err => alert("Xəta: " + err.message));
    };
    ...
    window.deleteBildiris = function(id) {
        ...
        fetch(`${BIL_API_URL}/${id}`, { method: 'DELETE' }).then(() => loadAllBildirisler(() => renderBildirisTable())).catch(err => alert("Xəta: " + err.message));
    };

    if (bildirisBtn && bildirisPopup) { 
        bildirisBtn.addEventListener('click', e => { e.preventDefault(); bildirisPopup.style.display = 'flex'; currentBilPage = 1; renderBildirisTable(); }); 
    }
    ...
    window.openBildirisPanelAndHighlight = function(bilId) {
        if (bildirisPopup) { bildirisPopup.style.display = 'flex'; renderBildirisTable(bilId); }
    };
    ...
    function fillFormWithData(company) {
        ...
        if (company.pstatus == 2) { if (radioPerson2) radioPerson2.checked = true; } 
        else { if (radioPerson1) radioPerson1.checked = true; }
        handleRadioChange(); 
    ...
    function renderTable() {
        ...
        pageData.forEach(company => {
            ...
            tr.querySelector('.btn-edit').addEventListener('click', (e) => { 
                e.preventDefault(); fillFormWithData(company); 
    ...
    function renderPagination(totalPages) {
        ...
        for (let i = startPage; i <= endPage; i++) {
            const pBtn = document.createElement('button'); 
            pBtn.className = `page-btn ${i === currentPage ? 'active' : ''}`; pBtn.innerText = i;
    ...
    if (refreshDbBtn) { 
        refreshDbBtn.addEventListener('click', (e) => { e.preventDefault(); loadCompanies(); clearFormFields(); }); 
    }
    if (addVoenBtn && popupDiv) { 
        addVoenBtn.addEventListener("click", (e) => { e.preventDefault(); popupDiv.style.display = "flex"; clearFormFields(); filterAndRenderTable(); }); 
    }
    if (closePopupBtn && popupDiv) { 
        closePopupBtn.addEventListener("click", (e) => { e.preventDefault(); popupDiv.style.display = "none"; loadCompanies(() => { refreshAnalysisIfPossible(); }); }); 
    }
    ...
    if (voenSrcIcon) { 
        voenSrcIcon.addEventListener('click', (e) => { e.preventDefault(); if (voenSrcIcon.classList.contains('active-search')) searchVoenAction(); }); 
    }
    if (inputVoen) {
        inputVoen.addEventListener('keypress', (e) => { if (e.key === 'Enter') { e.preventDefault(); if (inputVoen.value.trim()) searchVoenAction(); } });
    }
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', (e) => {
            ...
            saveBtn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> Yadda saxlanılır...`; saveBtn.disabled = true;

            const finalPayload = { 
                voen: voenVal, comp_name: pstatusVal === 2 ? '' : companyVal, 
                comp_director_name: leaderVal, comp_adress: addressVal, 
                pstatus: pstatusVal, data_info_date: window.getTodayFormatted() 
            };
    ...
    if (fileBtn && fileArea) {
        ...
        fileInput.addEventListener('change', () => { 
            if (fileInput.files.length > 0) { 
                selectedFile = fileInput.files[0]; fileArea.innerText = selectedFile.name; fileArea.style.color = "#3b82f6"; 
            } 
        });
    }
    ...
    sections.forEach(s => { 
        if (s.btn && s.div) { 
            s.btn.addEventListener('click', (e) => { 
                e.preventDefault(); sections.forEach(x => { if(x.div) x.div.classList.remove('active-filter'); }); 
                s.div.classList.add('active-filter'); currentFilterType = s.type; 
            }); 
        } 
    });
    ...
    if (analizBtn) {
        analizBtn.addEventListener('click', function(e) {
            e.preventDefault();
            if (typeof XLSX === 'undefined') { alert("Excel analiz kitabxanası (XLSX) tapılmadı!"); return; }
            if (!selectedFile || !currentFilterType) { alert("Zəhmət olmasa əvvəlcə faylı və dövrü seçin!"); return; }

            let displayPeriod = "", filterPeriod = "", filterYear = "";
            if (currentFilterType === "rub") {
                ...
                filterPeriod = rawPeriod.trim().toLowerCase(); filterYear = rawYear.trim();
                ...
                if(!baslangic || !son) { alert("Başlanğıc və son tarixi tam seçin!"); return; }
                displayPeriod = `${baslangic.split('-').reverse().join('.')} - ${son.split('-').reverse().join('.')}`;
            }

            if (neticeDovrElement) neticeDovrElement.innerText = displayPeriod;
            if(analizBox) analizBox.innerHTML = `<div style="text-align:center; padding: 40px;"><i class="fa-solid fa-circle-notch fa-spin" style="font-size:30px; color:#3b82f6;"></i><p style="margin-top:10px;">Analiz edilir, gözləyin...</p></div>`;
                    ...
                    try {
                        ...
                        for (let i = 1; i < excelData.length; i++) {
                            ...
                            if (currentFilterType === "tarix") {
                                ...
                                if (excelD && excelD >= new Date(baslangic + "T00:00:00") && excelD <= new Date(son + "T23:59:59")) isMatch = true;
                            }
                            ...
                            if (isMatch && qaliqBorc > minAmountFilter) {
                                ...
                                let isFiziki = (voen && voen.toString().endsWith('2')) || (!/mmc|məhdud məsuliyyətli cəmiyyət|asc|qsc/i.test(firmaAdiRaw));
                                if (voen && voen.toString().endsWith('1')) isFiziki = false;

                                if (!groupedResults[idareAdi]) groupedResults[idareAdi] = {};
                                let groupKey = voen ? voen : firmaAdiRaw;
                                if (!groupedResults[idareAdi][groupKey]) { 
                                    groupedResults[idareAdi][groupKey] = { firma: firmaAdiRaw, voen: voen || "", isFiziki: isFiziki, decls: {}, toplamBorc: 0, qeydSayi: 0 }; 
                                }
                        ...
                        if (Object.keys(groupedResults).length === 0) { 
                            if(analizBox) analizBox.innerHTML = `<div style="text-align:center; padding: 40px; color:#ef4444;"><i class="fa-solid fa-circle-exclamation" style="font-size: 30px; margin-bottom: 10px;"></i><p>Uyğun borc tapılmadı!</p></div>`; 
                            document.querySelectorAll('.action-bar .btn').forEach(btn => { btn.disabled = true; btn.style.opacity = '0.5'; btn.style.cursor = 'not-allowed'; });
                            return; 
                        }

                        let htmlContent = ``; let idareIdx = 1;
                        for (const idare in groupedResults) {
                            let idaredəkiFirmalar = Object.values(groupedResults[idare]);
                            let idareUmumiBorc = 0, idareUmumiQeydSayi = 0;
                            ...
                            idaredəkiFirmalar.forEach(item => {
                                let isVoenInDb = item.voen ? allCompaniesData.some(c => c.voen && c.voen.toString() === item.voen.toString()) : false;
                                let missingAlertHtml = !isVoenInDb && item.voen ? `<button class="add-missing-voen-btn" data-voen="${item.voen}" data-firma="${item.firma.replace(/"/g, '&quot;')}" data-isfiziki="${item.isFiziki}"><i class="fa-solid fa-triangle-exclamation"></i> VÖEN bazada yoxdur - Əlavə etmək üçün klikləyin</button>` : "";

                                let newDecls = [], oldDecls = [], newBorc = 0, newTarixler = new Set(), allTarixler = new Set(), accordionListHtml = ``, hasBildiris = false;
                                let foundRecordsForFirm = allBildirislerData.filter(b => (item.voen && b.voen === item.voen.toString()) || (!item.voen && b.firma === item.firma));

                                for(const nomre in item.decls) {
                                    let borcu = item.decls[nomre].borc;
                                    Array.from(item.decls[nomre].tarixler).forEach(t => allTarixler.add(t));
                                    let matchedRecord = foundRecordsForFirm.find(b => b.melumat && new RegExp(`\\b${nomre}\\b`).test(b.melumat));

                                    if (matchedRecord) {
                                        hasBildiris = true; oldDecls.push(nomre);
                                        ...
                                        newDecls.push(nomre); newBorc += borcu; 
                                        Array.from(item.decls[nomre].tarixler).forEach(t => newTarixler.add(t));
                                ...
                                let bgStyle = "#ffffff", cardBorder = "1px solid #e2e8f0", statusBadge = "";
                                if (oldDecls.length > 0 && newDecls.length > 0) {
                                    bgStyle = "#fffbeb"; cardBorder = "1px solid #fde68a";
                                    statusBadge = `<div class="badge-pill" style="background:#fef3c7; color:#d97706;"><i class="fa-solid fa-code-merge"></i> Qismən Yeni</div>`;
                                } else if (oldDecls.length > 0 && newDecls.length === 0) {
                                    bgStyle = "#f8fafc"; cardBorder = "1px solid #e2e8f0";
                                    statusBadge = `<div class="badge-pill" style="background:#e2e8f0; color:#475569;"><i class="fa-solid fa-database"></i> Tamamilə Bildiriş Yazılıb</div>`;
                                } else {
                                    bgStyle = "#f0fdf4"; cardBorder = "1px solid #bbf7d0";
                                    statusBadge = `<div class="badge-pill" style="background:#dcfce7; color:#166534;"><i class="fa-solid fa-sparkles"></i> Yeni Bildiriş</div>`;
                                }
                                ...
                                htmlContent += `<div class="firm-item" style="background: ${bgStyle}; border: ${cardBorder}; opacity: ${opacityStyle};"><div class="firm-main-row"><input type="checkbox" class="custom-checkbox firma-check2" ${checkboxAttr} data-idare="${safeIdare}" data-voen="${item.voen}" data-firma="${item.firma.replace(/"/g, '&quot;')}" data-isfiziki="${item.isFiziki}" data-gb="${Object.keys(item.decls).join(", ")}" data-new-gb="${newDecls.join(', ')}" data-old-gb="${oldDecls.join(', ')}" data-borc="${item.toplamBorc.toFixed(2)}" data-new-borc="${newBorc.toFixed(2)}" data-new-tarixler="${Array.from(newTarixler).join(", ")}"><div class="firm-info"><div class="firm-name">${item.firma} <span class="firm-voen">(VÖEN: ${item.voen || "Yoxdur"})</span></div><div class="firm-badges"><div class="badge-pill"><strong>Yeni Bəyannamələr:</strong> ${newDecls.length > 0 ? newDecls.join(', ') : 'Yoxdur'}</div><div class="badge-pill badge-danger-pill">Yeni Borc: ${newBorc.toFixed(2)} ABŞ</div>${statusBadge}</div>${missingAlertHtml}</div>${hasBildiris ? `<button class="toggle-btn" title="Bəyannamələri göstər"><i class="fa-solid fa-chevron-down"></i></button>` : ''}</div>${hasBildiris ? `<div class="details-panel"><ul>${accordionListHtml}</ul></div>` : ''}</div>`;
                            });
                            htmlContent += `</div>`; idareIdx++;
                        }
                        if(analizBox) analizBox.innerHTML = htmlContent;

                        document.querySelectorAll('.action-bar .btn').forEach(btn => { btn.disabled = false; btn.style.opacity = '1'; btn.style.cursor = 'pointer'; });
                        ...
                        document.querySelectorAll('.add-missing-voen-btn').forEach(btn => {
                            btn.addEventListener('click', function(e) {
                                ...
                                clearFormFields();
                                if (inputVoen) { inputVoen.value = this.getAttribute('data-voen'); inputVoen.dispatchEvent(new Event('input')); }
                                if (this.getAttribute('data-isfiziki') === 'true') { if(radioPerson2) radioPerson2.checked = true; if(inputLeader) inputLeader.value = this.getAttribute('data-firma'); } 
                                else { if(radioPerson1) radioPerson1.checked = true; if(inputCompany) inputCompany.value = this.getAttribute('data-firma'); }
                                handleRadioChange();
                            });
                        });
                        ...
                        document.querySelectorAll('.toggle-btn').forEach(btn => {
                            btn.addEventListener('click', function(e) {
                                e.preventDefault();
                                const panel = this.closest('.firm-item').querySelector('.details-panel');
                        ...
                        document.querySelectorAll('.idare-check1').forEach(idareCheck => { 
                            idareCheck.addEventListener('change', function() { 
                                this.closest('.result-group').querySelectorAll('.firma-check2:not([disabled])').forEach(child => child.checked = this.checked); 
                            }); 
                        });
    ...
    if(closePrezipBtn) closePrezipBtn.addEventListener('click', () => { if(preZipPopup) preZipPopup.style.display = 'none'; });
    if(cancelPrezipBtn) cancelPrezipBtn.addEventListener('click', () => { if(preZipPopup) preZipPopup.style.display = 'none'; });
    if(closeZipPopupBtn) closeZipPopupBtn.addEventListener('click', () => { if(zipPopup) zipPopup.style.display = 'none'; });
    if(cancelZipSaveBtn) cancelZipSaveBtn.addEventListener('click', () => { if(zipPopup) zipPopup.style.display = 'none'; });
    
    if(zipSelectAll) { 
        zipSelectAll.addEventListener('change', (e) => { document.querySelectorAll('.zip-row-check').forEach(cb => cb.checked = e.target.checked); }); 
    }

    // 5. ZIP HAZIRLAMA (EXECUTE ZIP) PROSESİ
    window.executeZipProcess = async () => {
        ...
        try {
            ...
            if(progressText) { progressText.innerText = '100% - Yüklənir!'; progressText.style.color = '#10b981'; }
            ...
            setTimeout(() => {
                ...
                if(progressText) { progressText.innerText = '0%'; progressText.style.color = '#3b82f6'; }
            }, 300);
        }
    };
    ...
    if(confirmPrezipBtn) {
        confirmPrezipBtn.addEventListener("click", () => {
            if (window.pendingFirmsToZip && window.pendingFirmsToZip.length === 0) {
                alert("Diqqət: Seçdiyiniz firmalar üçün yeni bildiriş yazılası bəyannamə yoxdur.");
    ...
    if (bildirisQosmaBtn) {
        bildirisQosmaBtn.addEventListener("click", function(e) {
            ...
            if (checkedFirms.length === 0) { alert("Diqqət: Zəhmət olmasa ən azı bir firma seçin!"); return; }

            let firmsToProcess = [], warningHtml = "";
            for (const checkbox of checkedFirms) {
                ...
                if (newGb.trim() !== "") {
                    warningHtml += `<tr><td><strong>${firmaAdi}</strong></td><td style="color:#ef4444; font-size:12px;">${checkbox.getAttribute("data-old-gb") || "Yoxdur"}</td><td style="color:#10b981; font-weight:bold; font-size:12px;">${newGb}</td><td style="font-weight:bold; color:#1e293b;">${newBorc} ABŞ</td></tr>`;
                    firmsToProcess.push({ checkbox: checkbox, newGb: newGb, newBorc: newBorc });
                }
            }

            if (firmsToProcess.length === 0) { alert("Seçdiyiniz firmaların bütün bəyannamələrinə artıq bildiriş yazılıb!"); return; }
    ...
    if(saveZipSelectionsBtn) {
        saveZipSelectionsBtn.addEventListener('click', function() {
            ...
            const finalPayload = [];
            checkedBoxes.forEach(cb => { finalPayload.push(pendingDbSavePayload[cb.getAttribute('data-idx')]); });
