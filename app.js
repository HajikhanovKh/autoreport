...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Düymələri Action Bar-a əlavə edirik
    const actionBar = document.querySelector('.action-bar');
    if (actionBar) {
        const coverSetBtn = document.createElement('button');
        coverSetBtn.className = 'btn';
        coverSetBtn.style.cssText = "background: #f8fafc; border: 1px solid #cbd5e1; color: #475569;";
        coverSetBtn.innerHTML = '<i class="fa-solid fa-envelope-open-text"></i> Zərf Ayarları';
        coverSetBtn.onclick = openCoverSettings;
        actionBar.appendChild(coverSetBtn);

        const coverGenBtn = document.createElement('button');
        coverGenBtn.className = 'btn';
        coverGenBtn.style.cssText = "background: #8b5cf6; border: 1px solid #7c3aed; color: white;";
        coverGenBtn.innerHTML = '<i class="fa-solid fa-print"></i> Zərf Üzlüklərinin Hazırlanması';
        coverGenBtn.onclick = openCoverGenerateModal;
        actionBar.appendChild(coverGenBtn);
    }

    // 1. Zərf Ayarları Modalı
    function openCoverSettings(e) {
        e.preventDefault();
        let modal = document.getElementById('cover-settings-modal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'cover-settings-modal';
            modal.style.cssText = "position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(4px); z-index: 99999; display: flex; justify-content: center; align-items: center;";
            modal.innerHTML = `
                <div style="background: white; width: 100%; max-width: 500px; border-radius: 16px; padding: 24px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1);">
                    <h2 style="margin-top:0; color:#0f172a; font-size:18px;"><i class="fa-solid fa-gear" style="color:#8b5cf6;"></i> Zərf Ayarları</h2>
                    <div id="cover-view-mode" style="margin: 20px 0;">
                        <p style="font-size:13px; color:#64748b; margin-bottom:4px;">Gömrük orqanının adı:</p>
                        <div id="cv-go" style="font-weight:600; color:#1e293b; padding:10px; background:#f8fafc; border-radius:8px; margin-bottom:12px;">Yüklənir...</div>
                        <p style="font-size:13px; color:#64748b; margin-bottom:4px;">Gömrük orqanının ünvanı:</p>
                        <div id="cv-adres" style="font-weight:600; color:#1e293b; padding:10px; background:#f8fafc; border-radius:8px;">Yüklənir...</div>
                    </div>
                    <div id="cover-edit-mode" style="margin: 20px 0; display:none;">
                        <label style="font-size:13px; font-weight:600; color:#475569;">Gömrük orqanının adı:</label>
                        <input type="text" id="ci-go" class="modal-input" style="width:100%; margin-bottom:12px; padding:10px; border:1px solid #cbd5e1; border-radius:8px;">
                        <label style="font-size:13px; font-weight:600; color:#475569;">Gömrük orqanının ünvanı:</label>
                        <input type="text" id="ci-adres" class="modal-input" style="width:100%; padding:10px; border:1px solid #cbd5e1; border-radius:8px;">
                    </div>
                    <div style="display:flex; justify-content:flex-end; gap:10px; border-top:1px solid #e2e8f0; padding-top:16px;">
                        <button id="btn-cover-close" style="padding:8px 16px; border:none; background:#e2e8f0; color:#475569; border-radius:8px; cursor:pointer;">Bağla</button>
                        <button id="btn-cover-edit" style="padding:8px 16px; border:none; background:#3b82f6; color:white; border-radius:8px; cursor:pointer;">Düzəliş Et</button>
                        <button id="btn-cover-save" style="padding:8px 16px; border:none; background:#10b981; color:white; border-radius:8px; cursor:pointer; display:none;">Saxla</button>
                        <button id="btn-cover-cancel" style="padding:8px 16px; border:none; background:#ef4444; color:white; border-radius:8px; cursor:pointer; display:none;">Ləğv Et</button>
                    </div>
                </div>`;
            document.body.appendChild(modal);

            document.getElementById('btn-cover-close').onclick = () => modal.style.display = 'none';
    ...
    function openCoverGenerateModal(e) {
        ...
        let modal = document.getElementById('cover-gen-modal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'cover-gen-modal';
            modal.style.cssText = "position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(4px); z-index: 99999; display: flex; justify-content: center; align-items: center;";
            document.body.appendChild(modal);
        }
        ...
        modal.innerHTML = `
            <div style="background: white; width: 100%; max-width: 500px; border-radius: 16px; padding: 24px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1);">
                <h2 style="margin-top:0; color:#0f172a; font-size:18px;"><i class="fa-solid fa-print" style="color:#8b5cf6;"></i> Zərf Sənədi Hazırlanır</h2>
                <p style="font-size:13px; color:#475569; margin-bottom:16px;">Aşağıdakı <strong>${window.pendingCovers.length}</strong> qeyd üçün tək bir Word faylında zərf üzlükləri yaradılacaq:</p>
                <ul style="list-style:none; padding:0; margin:0; max-height:250px; overflow-y:auto;">${listHtml}</ul>
                <div style="display:flex; justify-content:flex-end; gap:10px; border-top:1px solid #e2e8f0; margin-top:16px; padding-top:16px;">
                    <button onclick="document.getElementById('cover-gen-modal').style.display='none'" style="padding:10px 16px; border:none; background:#e2e8f0; color:#475569; border-radius:8px; cursor:pointer;">Ləğv Et</button>
                    <button id="btn-execute-cover" style="padding:10px 20px; border:none; background:#8b5cf6; color:white; font-weight:bold; border-radius:8px; cursor:pointer;"><i class="fa-solid fa-file-word"></i> Hazırla və Yüklə</button>
                </div>
            </div>`;
        modal.style.display = 'flex';
