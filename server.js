...
app.post('/api/companies', async (req, res) => {
    ...
    try {
        ...
        if (existing.length > 0) {
            ...
            return res.json({ success: true, updated: true, message: 'Məlumatlar yeniləndi!' });
        } else {
            const insertQuery = `
            ...
            return res.json({ success: true, updated: false, message: 'Yeni məlumat əlavə edildi!' });
        }
    } catch (err) {
        res.status(500).json({ error: err.message });
    } finally {
...
app.listen(port, () => {
    console.log(`Server ${port} portunda tam aktivdir və yüngülləşdirilmişdir.`);
});
